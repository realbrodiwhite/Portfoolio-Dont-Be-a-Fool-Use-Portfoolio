...
import { PlatformService } from '@portfoolio/api/app/platform/platform.service';
...
import { PropertyService } from '@portfoolio/api/services/property/property.service';
import { TagService } from '@portfoolio/api/services/tag/tag.service';
import {
  DEFAULT_CURRENCY,
...
export class InfoService {
  ...
  public async get(): Promise<InfoItem> {
    ...
    if (this.configurationService.get('ENABLE_FEATURE_FEAR_AND_GREED_INDEX')) {
      if (this.configurationService.get('ENABLE_FEATURE_SUBSCRIPTION')) {
        ...
        info.fearAndGreedDataSource = portfoolioFearAndGreedIndexDataSource;
      }
  ...
  private async countDockerHubPulls(): Promise<number> {
    try {
      ...
      const { pull_count } = await got(
        `https://hub.docker.com/v2/repositories/portfoolio/portfoolio`,
        {
  ...
  private async countGitHubContributors(): Promise<number> {
    try {
      ...
      const { body } = await got('https://github.com/portfoolio/portfoolio', {
        // @ts-ignore
        signal: abortController.signal
      });
      ...
      return extractNumberFromString({
        value: $(
          `a[href="/portfoolio/portfoolio/graphs/contributors"] .Counter`
        ).text()
      });
    } catch (error) {
  ...
  private async countGitHubStargazers(): Promise<number> {
    try {
      ...
      const { stargazers_count } = await got(
        `https://api.github.com/repos/portfoolio/portfoolio`,
        {
          headers: { 'User-Agent': 'request' },
          // @ts-ignore
          signal: abortController.signal
